...
export function observationFromScenario(data) {
  const model = {
  ...
  const satellites = (data.satellites || []).map((sat) => enrichSatFromFile(sat, step, model));
  const jobs = (data.jobs || []).map((job) => enrichJobFromFile(job, step));
...
function enrichSatFromFile(sat, step, model) {
  ...
  const energy_wh = sat.energy_wh ?? 0;
  const soc = (100 * energy_wh) / capacity_wh;
  const temp_c = sat.temp_c ?? 0;
  ...
  if (!action) {
    ...
    else if ((sat.calibration_age_steps ?? 0) > model.calibration_valid_steps) {
  ...
  return {
    ...
    energy_wh,
    temp_c,
    calibration_age_steps: sat.calibration_age_steps ?? 0,
    available,
    action,
    job_id,
    idle_reason,
