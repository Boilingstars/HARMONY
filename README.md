...
Kepler и Basilisk остаются в `sim/`, но в `Session.advance` не вызываются: в описании данных сказано прямо, что координаты и станции не считаются, Солнце и доступность уже заданы рядами. Kepler нужен для будущего глобуса в `web/` и для генерации дополнительных сценариев в схеме `cosmo-B-ops-1.0`.

## Сценарии

| Файл | Состав | Роль |
|------|--------|------|
| [P01_intro.json](Кейс/data/P01_intro.json) | 16 КА, 48 шагов, 18 заданий | дымовые тесты, отладка масок |
| [P02_shift.json](Кейс/data/P02_shift.json) | 48 КА, 288 шагов, 1208 заданий | основное обучение и бейзлайн |
| [P03_energy.json](Кейс/data/P03_energy.json) | 48 КА, дефицит энергии | оценка |
| [P04_demand.json](Кейс/data/P04_demand.json) | 8120 заданий | поздний стресс-тест |
...
На оценке вместо случайного шума подаётся сценарий [events_demo.json](Кейс/examples/events_demo.json) (`--events demo`) либо шум отключается (`--events none`).
...
Кандидаты слотов действия — top-K всего известного списка: сначала задания, которые можно исполнить сейчас, затем ближайшие ещё не открытые. У будущих маска выключена, признаки видны. Назначить задание до окна нельзя. Полный список при этом лежит в `plan`, поэтому обрезка до 32 не прячет план. На P02 одновременно открыто в среднем 47 заданий; `--top-k` поднимает число слотов, не длину `plan`.
...
**Почему repair вне сети.** Жёсткие ограничения дешевле починить детерминированно, чем выучивать. Это совпадает с поведением `Environment.step`, который сам гасит `duplicate_job_in_step` и `ground_capacity` в `idle`, но чинится **до** отправки, чтобы в журнал смены не попадали заведомо отклонённые команды. Тайбрейк при конфликте: продолжение прежнего исполнителя, затем запас энергии, затем номер аппарата; при переполнении downlink-слотов остаются задания с более высоким приоритетом, близким дедлайном и большей стоимостью. Инварианты закреплены тестами `test_third_downlink_in_a_tick_becomes_idle` и `test_unique_job_per_step_and_downlink_cap`.
...
Модель: `agent/models/ops_<goal>.zip`. Полезные флаги: `--n-envs`, `--subproc`, `--top-k`, `--max-steps`, `--no-events`, `--tensorboard`, `--seed`.
...
agent/runs/                   логи обучения (в git не идут)
agent/figures/                графики и сводки
web/                          заготовка браузерной 3D-визуализации
```
